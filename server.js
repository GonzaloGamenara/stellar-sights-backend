...
async function fetchPlanets(system) {
    const planetsUrl = `https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+pl_name,+disc_year,+pl_bmasse,+pl_eqt,+pl_rade,+sy_dist,+pl_orbper,+pl_dens,+pl_telescope,+pl_ratror,+pl_mnum+from+ps+where+hostname='${system}'+and+default_flag=1&format=json`;
